...
def d_time(args):
    ...
    site_nodes=[ox.get_nearest_node(graph,(args['lat_'+str(i)],args['lon_'+str(i)]) ) for i in range(n_sites)]
    ...
    return np.sum(times)/np.sum(data)/60.+np.std(site_pop)/np.sum(data) #Not sure what to put as 14 here
 

#trials = MongoTrials('mongo://localhost:1234/zip_db/jobs', exp_key=county.replace(' ','_')+'_sitesv2_'+str(n_sites))
trials= Trials()

res=fmin(d_time,hp_space, algo=tpe.suggest,max_evals=10000,trials=trials)
